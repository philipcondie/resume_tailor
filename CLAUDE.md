...
3. **Preview & print** — The Preview page (`src/pages/Preview.tsx`) renders a print-ready resume. Sections are driven by a **section registry** (`src/types/SectionRegistry.ts`) that maps `ResumeData` keys to React components. Section visibility and ordering are user-configurable via `useLayoutConfig` and the `SectionSelector` component. An overflow detector warns if content exceeds one page.

### Key patterns

- **`useLocalStorage<T>` hook** (`src/hooks/useLocalStorage.ts`) — generic hook used by all data hooks; supports functional updates.
- **Section registry** — `sectionRegistry` maps each `keyof ResumeData` to a component implementing `SectionProps`. Adding a new resume section means: define the type in `resume.ts`, create the component, and register it.
