...
- **Linkable text** — `LinkableText` in `src/types/resume.ts` is `{ text: string, url: string | null }`. Project `title` and each `personalInfo.extras` entry use this shape. `src/lib/links.ts` normalizes URLs (HTTP/HTTPS only, adding `https://` when omitted) and upgrades legacy string values returned for profiles or saved resumes. Keep this compatibility normalization at API boundaries when adding new read/update paths.
